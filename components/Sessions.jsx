  ...
  return (
    <div className='p-6 px-12 md:px-24 lg:px-24 py:20 md:py-32 w-full font-rubik pb-10'>
      <div className="flex justify-between items-center sticky md:relative top-24 md:top-0">
        <h1 className='text-2xl font-bold font-rubik text-primary text-center md:text-left pb-6 uppercase '>News & Events</h1>
        <Link href={'/news'} className='flex items-center text-xs md:text-sm '>View More <span className='animate-pulse'><IoMdArrowForward size={20} /> </span></Link>
      </div>

      <div className='grid md:grid-cols-4 grid-cols-1 md:space-y-0 space-y-5 gap-[1.5rem] md:gap-[1.2rem] lg:gap-[1.6rem] pb-6'>
        {images.length > 0 && images.slice(0, 4).map((image, i) => (
          <div key={i} className='sticky md:relative top-[9rem] md:top-0 overflow-hidden flex flex-col flex-grow bg-yellow-50 rounded-xl shadow md:hover:shadow-lg md:hover:scale-105 duration-150 w-full'>
            <Image
              src={image.imageUrl}
              alt={image.title || 'Session'}
              width={0}
              height={0}
              sizes="100vw"
